...
import CirclePhoneMockup from "./CirclePhoneMockup";
import ImagePhoneMockup from "./ImagePhoneMockup";
import LinkPhoneMockup from "./LinkPhoneMockup";
    ...
    return (
        <div className="hidden bg-white rounded-xl p-5  lg:block lg:w-[30rem]">
            <svg className="mx-auto" xmlns="http://www.w3.org/2000/svg" width="308" height="632" fill="none" viewBox="0 0 308 632">
                ...
                {user.email ? <TextDataUser x="50%" y="214">{user.email}</TextDataUser> : <Rect width="72" height="8" x="117.5" y="214" fill="#EEE" rx="4" />}

                {links.map((link, i) => {
                    if (i===5)return 
                    const yPos = 278 + (i * 64);
                    return <LinkPhoneMockup link={link} key={i} x="35" y={yPos} />;
                })}
                ...
                Array.from({length:5}).map((_,i)=>{
                    const yPos = 278 + (i * 64);
                    return <Rect key={i} width="237" height="44" x="35" y={yPos} fill="#EEE" rx="8" />
                })
                }
            </svg>
        </div>
...
export default PreviewLinksHome
